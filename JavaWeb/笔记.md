...
![浏览器访问Web资源的流程图](photo\浏览器访问Web资源的流程图.png)
...
![jar参数](photo\jar参数.bmp)

### 打包

![WEB应用打包](photo\WEB应用打包.bmp)
...
6. WEB应用找到请求对应的WEB资源
   ...
   ![img](photo/Tomcat%E4%BD%93%E7%B3%BB%E6%9E%B6%E6%9E%84.bmp?lastModify=1524404941)
...
![用加密连接器访问WEB应用](photo\用加密连接器访问WEB应用.bmp)
...
![Servlet的声明周期](photo\Servlet的声明周期.jpg)
...
![客户端访问Servlet流程图](photo\客户端访问Servlet流程图.jpg)
...
- 使用错误码表再次编码后在使用正确码表解码
  ...
  name=new String(name.getBytes("错误码表(ISO-8859-1)"),"正确码表(UTF-8)")
  ```
...
![out隐式对象的工作原理](photo\out隐式对象的工作原理.bmp)
...
![out隐式对象的工作原理02](photo\out隐式对象的工作原理02.bmp)
...
![out隐式对象的工作原理01](photo\out隐式对象的工作原理01.bmp)
...
<%
    ...
    s.getBytes();		                      //调用null对象方法,将会出现空指针异常
    response.getWriter().write("HelloWrold：");//输出内容
%>
</body>
</html>
...
![errorPage01](photo\errorPage01.bmp)
...
![errorPage02](photo\errorPage02.bmp)
...
![pageContext](photo\pageContext.bmp)
...
![自定义标签_TagSupport](photo\自定义标签_TagSupport.bmp)
...
![自定义标签控制JSP内容](photo\自定义标签控制JSP内容.bmp)
...
![自定义标签控制JSP内容01](photo\自定义标签控制JSP内容01.bmp)
...
![自定义标签控制JSP内容02](photo\自定义标签控制JSP内容02.bmp)
...
![自定义标签控制JSP内容03](photo\自定义标签控制JSP内容03.bmp)
...
![自定义标签控制JSP内容04](photo\自定义标签控制JSP内容04.bmp)
...
![自定义标签控制JSP内容05](photo\自定义标签控制JSP内容05.bmp)
...
![body-content属性值_empty](photo\body-content属性值_empty.bmp)
...
- **JSP：接受所有JSP语法，如定制的或内部的tag、scripts、静态HTML、脚本元素、JSP指令和动作。** 
  ...
  ![body-content属性值_JSP](photo\body-content属性值_JSP.bmp)
  ...
  ![body-content属性值_JSP01](photo\body-content属性值_JSP01.bmp)
  ...
  ![body-content属性值_scriptless](photo\body-content属性值_jsp06.bmp)
...
- **tagdependent：标签体内容直接被写入BodyContent，由自定义标签类来进行处理，而不被JSP引擎解释. **
  ...
  ![body-content属性值_tagdependent](photo\body-content属性值_tagdependent.bmp)
...
- scriptless: 标签体可以包含EL表达式和JSP动作元素，但不能包含JSP的脚本元素
  ...
  ![body-content属性值_scriptless](photo\body-content属性值_scriptless.bmp)
...
![设置自定义标签参数01](photo\设置自定义标签参数01.bmp)
...
- **从Web域中获取数据**
  ...
  > web域（pageContext，request，session，servletContext）中检索指定 name的值,按照域大小查询，查到返回指定值，查不到返回““（空字符）

  ```
  <%-- 向pageContext设置属性 --%>
  <% pageContext.setAttribute("age", "17"); %>
  <%-- 使用el表达式获取pageContext中的属性 --%>
  <c:out value='${age}' default="name"></c:out>
...
![EL隐式对象](photo\el表达式运算符.bmp)

- **在el表达式中获取隐式对象信息**

  ![EL隐式对象](photo\EL隐式对象.bmp)
  ...
  <body>
  <!-- 在request域中获取name作为key保存的参数 -->
  <c:out value='${param.name}'></c:out>
  <!-- 在request域中获取name作为key保存的参数，找不到返回默认值jr -->
  ...
  ![el表达式_获取请求参数](photo\el表达式_获取请求参数.bmp)

  ​	**parma.key名 底层是通过调用request.getParma（key名）在request域中获取请求参数**
...
- **调用静态Java方法**
  ...
  **两者执行效果**
  ...
  ![el表达式_执行函数](photo\el表达式_执行函数.bmp)
...
page指令中确少 isELIgnored="false" 加上就好了
...
3. 解压压缩包
   ...
   ![jstl压缩包](photo\jstl压缩包.bmp)

4. 将lib目录下的两个jar拷到本地项目中的lib目录下并引用这两个jar包

   ![引入JSTLjar包](photo\引入JSTLjar包.bmp)
...
![Filter过滤器描述](photo\Filter过滤器描述.bmp)
